...
/**
 * Oyun Entegrasyon Yardımcısı
 * Swipe navigasyon sistemi dahil çeşitli altyapıyı oyunlarla entegre eder
 */

// DOM yüklendikten sonra çalışacak
document.addEventListener('DOMContentLoaded', function() {
  // Swipe navigasyon sistemi için durum yönetimi
  window.SwipeNavigationHelper = {
    // Oyun içinde swipe navigasyonu geçici olarak devre dışı bırakır
    disableSwipe: function() {
      document.body.classList.add('swipe-disabled');
      const swipeElements = document.querySelectorAll('.swipe-overlay, .edge-shadow, .prev-page-preview');
      swipeElements.forEach(elem => {
        if (elem) elem.style.display = 'none';
      });
    },
    
    // Swipe navigasyonu tekrar etkinleştirir
    enableSwipe: function() {
      document.body.classList.remove('swipe-disabled');
    },
    
    // Özel bir geri bağlantısı ekler - menüye dönüş için
    addBackToMenuLink: function(elementId, text = 'Oyundan Çık') {
      const element = document.getElementById(elementId);
      if (!element) return;
      
      const backLink = document.createElement('a');
      backLink.href = '/all_games';
      backLink.className = 'modern-back-link';
      backLink.innerHTML = `
      ...
      backLink.addEventListener('mouseenter', function() {
        backLink.style.background = 'rgba(0,0,0,0.3)';
        backLink.style.transform = 'translateX(-3px)';
      });
      
      backLink.addEventListener('mouseleave', function() {
        backLink.style.background = 'rgba(0,0,0,0.2)';
        backLink.style.transform = 'translateX(0)';
      });
      
      // Element başına ekle
      element.insertBefore(backLink, element.firstChild);
      
      // Tıklama - özel geçmiş yönetimi
      backLink.addEventListener('click', function(e) {
        e.preventDefault();
        
        // Çıkış animasyonu
        document.body.style.transition = 'transform 0.3s cubic-bezier(0.2, 0.85, 0.4, 1), opacity 0.3s ease';
        document.body.style.transform = 'translateX(100%)';
        document.body.style.opacity = '0.8';
        
        // Geçmişi güncelle ve ana sayfaya dön
        setTimeout(function() {
          let history = JSON.parse(localStorage.getItem('siteHistory') || '[]');
          
          // Ana sayfada değilse, geçmişten son sayfayı çıkar ve ana sayfayı ekle
          if (history.length > 0 && history[history.length - 1].url !== '/all_games') {
            history.pop(); // Mevcut oyun sayfasını çıkar
            
            // Eğer all_games zaten geçmişte varsa, oraya kadar kes
            const gamesPageIndex = history.findIndex(page => page.url === '/all_games');
            if (gamesPageIndex !== -1) {
              history = history.slice(0, gamesPageIndex + 1);
            } else {
              // Yoksa ana sayfayı ekle
              history.push({
                url: '/all_games',
                title: 'Tüm Oyunlar',
                timestamp: Date.now()
  ...
  // Swipe navigasyon sistemi oyunlar için devre dışı bırakılabilir
  // Oyun sayfalarında isteğe bağlı olarak çağrılabilir
  const gameContainers = document.querySelectorAll('.game-container, .game-view, .game-fullscreen');
  gameContainers.forEach(container => {
    // Oyun aktif olduğunda swipe'ı opsiyonel olarak devre dışı bırak
    // Bu tamamen isteğe bağlı, bazı oyunlar için gerekebilir
    if (container.hasAttribute('data-disable-swipe')) {
      window.SwipeNavigationHelper.disableSwipe();
    }
  });
});
