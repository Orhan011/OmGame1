...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const gameOverScreen = document.getElementById('labyrinthGameOver'); // Renamed for clarity
  ...
  const messageContainer = document.getElementById('labyrinthMessages'); // Moved here for better scope
  ...
  let gameStartTime; // Added to track game start time
  let moves = 0; // Added to track moves
  ...
  function checkGoal() {
    if (
      ...
      // Standardize edilmiş puan hesaplama sistemini kullan (varsayılan olarak basit bir hesaplama)
      const scoreParams = {
        gameType: 'labyrinth',
      ...
      let scoreDetails = calculateScore(scoreParams); // Use the new function
  ...
  function showLevelCompleteMessage(scoreDetails) {
    ...
    message.innerHTML = `
      ...
      <p>Puan: ${scoreDetails.finalScore}</p>
  ...
  function endGame() {
    ...
    const gameStats = {
      totalMoves: moves,
      difficulty: difficulty, // Using the actual difficulty level
    ...
    const scoreParams = {
      ...
      optimalTime: difficultyToTime(difficulty, currentLevel) * 0.6,
      totalMoves: gameStats.totalMoves,
      correctMoves: gameStats.totalMoves,
      hintsUsed: 0,
      level: currentLevel,
      maxLevel: 5,
      gameSpecificStats: gameStats
    };
    let scoreDetails = calculateScore(scoreParams); // Use the calculateScore function


    // Skoru arka planda kaydet, ekranda göstermeden
    saveScore(scoreDetails.finalScore, gameStats);
    ...
    gameOverScreen.classList.remove('d-none'); // Show game over screen
    finalScoreDisplay.textContent = scoreDetails.finalScore; // Display final score
  ...
  // Skoru kaydetme fonksiyonu - API çağrısı veya alternatif yöntem
  function saveScore(finalScore, gameStats) {
    try {
      // İsteğe göre loglama
      console.log(`Saving score for labyrinth: ${finalScore} points`);
      //  API çağrısını burada gerçekleştir
      fetch('/api/save-score', {
          ...
          body: JSON.stringify({
            game_type: 'labyrinth',
            score: finalScore,
            difficulty: difficulty, // Send actual difficulty
            playtime: gameStats.timeSpent,
  ...
  //  A simple score calculation function -  REPLACE THIS WITH YOUR ACTUAL SCORE CALCULATION LOGIC
  function calculateScore(params) {
    // This is a placeholder; replace with your actual scoring algorithm
    let baseScore = 40;
    let timeBonus = Math.max(0, Math.min(40, Math.floor((params.optimalTime - params.timeSpent) / 5)));
    let movePenalty = Math.max(0, params.totalMoves - params.optimalTime) * 2;
    let difficultyMultiplier = 1;
    if (params.difficulty === 'medium') difficultyMultiplier = 1.2;
    else if (params.difficulty === 'hard') difficultyMultiplier = 1.5;
    let finalScore = Math.max(0, baseScore + timeBonus - movePenalty) * difficultyMultiplier;
    return { finalScore: Math.round(finalScore), breakdown: { baseScore, timeBonus, movePenalty, difficultyMultiplier } };
  }


  // Show game over message
  function showGameOverMessage(finalScore, timeElapsed) {
    const message = document.createElement('div');
    message.className = 'game-over-message';
    message.innerHTML = `
      <h3>Oyun Bitti!</h3>
      <p>Puanınız: ${finalScore}</p>
      <p>Süreniz: ${timeElapsed} saniye</p>
