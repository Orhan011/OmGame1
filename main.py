...
def user_profile(user_id):
    ...
    try:
        ...
        # Seviye ilerleme yüzdesini hesapla
        next_level_xp = xp_for_level(user_level + 1)
        current_level_xp = xp_for_level(user_level)
        xp_needed = next_level_xp - current_level_xp
        xp_progress = ((user.experience_points - current_level_xp) / xp_needed) * 100 if xp_needed > 0 else 100

        # Kullanıcının en yüksek skorlarını al
        user_scores = {}
        scores = Score.query.filter_by(user_id=user.id).order_by(Score.score.desc()).all()
        
        for score in scores:
            if score.game_type not in user_scores or score.score > user_scores[score.game_type]:
                user_scores[score.game_type] = score

        # Kullanıcının son 10 skorunu al
        recent_scores = Score.query.filter_by(user_id=user.id).order_by(Score.timestamp.desc()).limit(10).all()

        # Oyun türlerinin Türkçe isimlerini içeren sözlük
        game_names = {
        ...
        flash('Kullanıcı profili yüklenirken bir hata oluştu.', 'danger')
        return redirect(url_for('index'))
