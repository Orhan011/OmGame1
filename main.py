        ...
        if request.method == 'POST':
            ...
            <head>
                <style>
                    ...
                    .code {{ font-size: 24px; font-weight: bold; color: #4e73df; text-align: center; padding: 10px; 
                            margin: 20px 0; border: 1px dashed #4e73df; background-color: #f8f9fc; }}
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        <p>Merhaba,</p>
                        <p>Şifre sıfırlama talebinde bulundunuz. Şifrenizi sıfırlamak için aşağıdaki doğrulama kodunu kullanın:</p>
                        <div class="code">{verification_code}</div>
                        <p>Bu kod 30 dakika boyunca geçerli olacaktır.</p>
                        <p>Eğer bu talebi siz yapmadıysanız, lütfen bu e-postayı dikkate almayın.</p>
                    </div>
                    <div class="footer">
                        <p>Bu e-posta OmGame tarafından gönderilmiştir.</p>
                    </div>
                </div>
            </body>
            </html>
            ...
            # Mevcut e-posta gönderme fonksiyonunu kullan
...
def reset_code():
    try:
        ...
        if request.method == 'POST':
            # Form verilerini al (kod1, kod2, kod3, kod4 veya verification_code)
            verification_code = request.form.get('verification_code')
            
            # Eğer verification_code yoksa, dört ayrı input'tan gelen değerleri birleştir
            ...
            if user.reset_token == verification_code and datetime.utcnow() < user.reset_token_expiry:
...
def reset_password():
    try:
        ...
        if not user:
            flash('Geçersiz bağlantı veya bağlantının süresi dolmuş.', 'danger')
            return redirect(url_for('forgot_password'))
