...
# Ana sayfa oyun kartları API endpoints
@app.route('/api/pinned-games', methods=['GET'])
def get_pinned_games():
    """Kullanıcının ana sayfaya sabitlediği oyunları getiren API."""
    user_id = session.get('user_id')
    
    # Kullanıcı giriş yapmamışsa varsayılan oyunları göster
    if not user_id:
        default_games = ['word-puzzle', 'memory-cards', 'number-sequence', 'audio-memory']
        return jsonify({
            'success': True,
            'games': default_games
        })
    ...
    # Sabitlenmiş oyunları getir
    pinned_games = user.pinned_games or []
...
@app.route('/api/pin-game', methods=['POST'])
def pin_game():
    """Kullanıcının ana sayfasına oyun ekleyen API."""
    user_id = session.get('user_id')
    ...
    if not game_id:
        return jsonify({'success': False, 'message': 'Oyun ID\'si belirtilmedi'})
    
    # Mevcut sabitlenen oyunları al
    pinned_games = user.pinned_games or []
    
    # Oyun zaten eklenmiş mi kontrol et
    if game_id in pinned_games:
        return jsonify({
            'success': True,
            'message': 'Bu oyun zaten eklenmiş',
            'games': pinned_games
        })
    
    # Maksimum oyun sınırını kontrol et
    MAX_PINNED_GAMES = 4
    if len(pinned_games) >= MAX_PINNED_GAMES:
        return jsonify({
            'success': False,
            'message': f'En fazla {MAX_PINNED_GAMES} oyun ekleyebilirsiniz'
        })
    
    # Oyunu ekle
    pinned_games.append(game_id)
    user.pinned_games = pinned_games
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"Oyun eklenirken hata: {e}")
        return jsonify({
            'success': False,
            'message': 'Oyun eklenirken bir hata oluştu'
        })

@app.route('/api/unpin-game', methods=['POST'])
def unpin_game():
    """Kullanıcının ana sayfasından oyun kaldıran API."""
    user_id = session.get('user_id')
    ...
    # Oyun eklenmiş mi kontrol et
    if game_id not in pinned_games:
        return jsonify({
            'success': True,
            'message': 'Bu oyun zaten eklenmemiş',
            'games': pinned_games
        })
    
    # Oyunu kaldır
    pinned_games.remove(game_id)
    user.pinned_games = pinned_games
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Oyun başarıyla kaldırıldı',
            'games': pinned_games
        })
    except Exception as e:
        db.session.rollback()
        logger.error(f"Oyun kaldırılırken hata: {e}")
        return jsonify({
            'success': False,
            'message': 'Oyun kaldırılırken bir hata oluştu'
        })
