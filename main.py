...
def save_score():
    ...
    if not user_id:
        # Return a special response that will trigger a login prompt
        return jsonify({
            'success': False, 
            'message': 'Login required',
            'score': data.get('score', 0),
            'redirect': url_for('login', next=request.referrer, save_score=True)
        })
...
def get_scores(game_type):
    ...
    try:
        ...
        if game_type == 'all':
            ...
            game_types = ['wordPuzzle', 'memoryMatch', 'labyrinth', 'puzzle', 'numberSequence']
            all_scores = {}
        ...
        try:
            ...
            logger.error(f"Error in get_scores for {game_type}: {e}")
            return jsonify([])
